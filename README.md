...
- “标书合规性校验”“评标规则校验”和“全面校验”均可执行；全面校验按“合规检查 → 评标检查”串行编排，复用合规阶段已经生成的投标文件解析、检查产物和事实，不重复执行合规检查；
- “评标规则校验”从招标文件的 MinerU 结构块中提取评分规则和具有明确后果的否决性规则，并继续执行现有客观评分和否决规则流程；主观评分仍通过独立接口触发；
