...
- 招标文件要求提取默认调用项目现有 MinerU `/tasks` 服务链路；只有显式配置 `BID_CHECK_ALLOW_DOCX_FALLBACK=true` 时才允许开发/测试用 DOCX XML fallback；
...
- `PDF_TRANS_MINERU_URL`：项目现有 MinerU `/tasks` 服务地址；也兼容 `MINERU_API_URL`。
- `PDF_TRANS_MINERU_BACKEND`：后端，默认 `hybrid-engine`；也兼容 `MINERU_BACKEND`。
- `PDF_TRANS_MINERU_SERVER_URL`：`hybrid-http-client` 模式必须配置；也兼容 `MINERU_SERVER_URL`。
- `MINERU_API_KEY`：可选的 MinerU Bearer Key。
- `MINERU_COMMAND`：显式配置的旧版 MinerU 命令适配器，仅在未配置服务地址时使用；命令标准输出应为 JSON 数组，或包含 `blocks`/`content`/`items` 数组。
- `BID_CHECK_ALLOW_DOCX_FALLBACK`：默认 `false`。仅开发/测试显式设为 `true` 时才允许 DOCX XML fallback；缺少 MinerU 配置时正常业务会明确失败。
- `PDF_TRANS_MINERU_TIMEOUT_SECONDS`：MinerU 任务总超时，默认 1800 秒。
- `PDF_TRANS_MINERU_POLL_INTERVAL_SECONDS`：任务状态轮询间隔，默认 2 秒。
