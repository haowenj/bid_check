...
本次出现的 v2 `type` 为 `title`、`paragraph`、`list`、`table`、`image`。
...
本次 v2 块没有逐块 `page_idx`、`anchor` 或对象索引。外层组可用于保持顺序和定位，但当前适配器只把它写入 `metadata.source_position.page_group_index`，不把组序号猜作 `page_idx`。
...
| `block_type` | v2 `type` 稳定可见 | 直接映射；未知保留 `unknown` |
...
| `source_object_index` | v2/legacy 不提供；middle 有 `index`，但不是选定 v2 块的可证明关联键 | v2/legacy 为 null；middle 不数组对齐补入 |
...
第一版选择 v2 作为标准化主输入；legacy 仅在 v2 缺失或结构不支持时回退。middle、legacy、v2 只通过各自明确字段被消费，不按数组位置做三方合并。后续若 MinerU 版本增加块级 page、anchor、formula 或对象键，应先更新本分析文档与契约 fixture，再扩展转换器和测试。
