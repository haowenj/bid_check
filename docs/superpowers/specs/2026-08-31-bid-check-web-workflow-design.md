...
本轮不调用 MinerU、LLM、VL、OCR、embedding、rerank，不实现 RAG、真实文档解析、真实规则提取或真实合规判断，也不实现缓存。
...
- `extract_compliance_requirements(tender_file)`：返回固定结构的五项模拟合规性要求；
...
函数接口不包含缓存细节。未来可以在服务函数前增加缓存装饰层或代理层，而不改变编排器。
...
本轮不实现真实解析、规则提取、合规判断、评分、否决投标判断、缓存、任务队列或分布式执行。
...
完成后应能实际完成上传、模式选择、任务创建、并行前置步骤、模拟审查和结果展示。页面明确说明没有执行真实投标文件校验，代码及运行日志中不存在 MinerU、LLM、VL、OCR、embedding 或 rerank 调用。
